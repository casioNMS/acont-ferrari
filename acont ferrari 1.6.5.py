...
class SplashScreen:
    ...
    def __init__(self, root):
        ...
        # imagem
        img = Image.open("logo acont ferrari.png")
        img = img.resize((400,200))
        self.logo = ImageTk.PhotoImage(img)

        self.canvas = tk.Canvas(root, width=largura, height=altura,
                                bg="black", highlightthickness=0)
        self.canvas.pack()

        # imagem no centro
        self.canvas.create_image(largura/2, altura/2-20, image=self.logo)

        # texto no canto inferior direito
        self.canvas.create_text(
            largura-10,
            altura-10,
            text="by: dream the ronin studios",
            fill="white",
            font=("Arial",10),
            anchor="se"
        )
...
class Main:
    ...
    def __init__(self, root):
        ...
        self.root = root
        self.root.title("acont ferrari 1.6.4")
        self.root.geometry("1000x800")
        self.root.configure(bg="#202020")
        self.root.state("zoomed")

        mixer.init()
        mixer.music.load("windows-7-startup.mp3")
        mixer.music.play()
        ...
        # tk.Label(root, text="hora",bg="#202020",fg="white").grid(row=5)
        # self.hora_entry=tk.Entry(root)
        # self.hora_entry.grid(row=5, column=1)
    ...
    def atualizar_relatorio(self):
        ...
        total_cigarros = sum(marcas.values())

        for marca, qtd in sorted(marcas.items(), key=lambda x: x[1], reverse=True):

            porcentagem = (qtd / total_cigarros) * 100 if total_cigarros > 0 else 0

            self.relatorio.insert("end",
                f"{marca:12} | {porcentagem:.2f}% ({qtd} un)\n"
            )
        ...
        self.relatorio.insert("end",f"TOTAL | R${self.total:.2f}\n")
